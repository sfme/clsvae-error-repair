...
class mixedDataset(Dataset):
    def __init__(
        ...
        for col_name in cat_feat_names:
            ...
            self.cat_to_idx.append(
                (col_name, OrderedDict(zip(cats_list, range(len(cats_list)))))
            )

            # from code to category
            self.idx_to_cat.append(
                (col_name, OrderedDict(zip(range(len(cats_list)), cats_list)))
            )

        self.cat_to_idx = OrderedDict(self.cat_to_idx)
        self.idx_to_cat = OrderedDict(self.idx_to_cat)

        self.cat_name_to_idx = dict(
            [
                (col_name, self.df_dataset.columns.get_loc(col_name))
                for col_name in cat_feat_names
            ]
        )
        self.cat_idx_to_name = dict(
            [
                (self.df_dataset.columns.get_loc(col_name), col_name)
                for col_name in cat_feat_names
            ]
        )
        ...
        self.num_name_to_idx = dict(
            [
                (col_name, self.df_dataset.columns.get_loc(col_name))
                for col_name in num_feat_names
            ]
        )
        self.num_idx_to_name = dict(
            [
                (self.df_dataset.columns.get_loc(col_name), col_name)
                for col_name in num_feat_names
            ]
        ...
        feat_len = 0
        if cat_feat_names:
            feat_len = reduce((lambda x, y: x + y), map(len, self.cat_to_idx.values()))
        if num_feat_names:
            feat_len += len(num_feat_names)
        self.size_tensor_one_hot = feat_len
        ...
        for col_name in self.df_dataset.columns:
            if col_name in cat_feat_names:  # categorical
                self.feat_info.append(
                    (col_name, "categ", len(self.cat_to_idx[col_name]))
                )
    ...
    def from_raw_to_one_hot(self, sample_row):
        ...
        for col_idx, col_name in enumerate(self.df_dataset.columns):
            ...
            if col_name in self.cat_cols:
                ...
                ret_tensor[
                    idx_cur + self.cat_to_idx[col_name][sample_row[col_idx]]
                ] = 1.0
    ...
    def from_raw_to_index(self, sample_row):
        ...
        for col_name in self.num_cols:
            ...
            ret_tensor[self.num_name_to_idx[col_name]] = (
                sample_row[self.num_name_to_idx[col_name]] - mean_col
            ) / std_col

        for col_name in self.cat_cols:

            ret_tensor[self.cat_name_to_idx[col_name]] = self.cat_to_idx[col_name][
                sample_row[self.cat_name_to_idx[col_name]]
            ]
    ...
    def standardize_dataset(self, column, cat_change=True):
        ...
        if column.name in self.num_cols:
            column = (column - self.cont_means[column.name]) / self.cont_stds[
                column.name
            ]
        elif cat_change:
    ...
    def standardize_dataset_one_hot(self, df_data):
        ...
        ret_data = df_data.apply(
            lambda col: self.standardize_dataset(col, cat_change=False)
        )
    ...
    def from_index_to_raw(self, sample_row):
        ...
        for col_name in self.cat_cols:
            ...
            ret_tensor[self.cat_name_to_idx[col_name]] = self.idx_to_cat[col_name][
                int(sample_row[self.cat_name_to_idx[col_name]])
            ]
...
class mixedDatasetInstance(mixedDataset):
    def __init__(
        ...
        # self.semi_sup_data = semi_sup_data
        ...
        for col_name in self.cat_cols:
            ...
            cat_type_col = pd.api.types.CategoricalDtype(
                categories=list(self.cat_to_idx[col_name].keys())
            )
        ...
        if self.one_hot:
            self.df_dataset_instance_standardized = self.standardize_dataset_one_hot(
                self.df_dataset_instance
            )
        else:
            self.df_dataset_instance_standardized = self.df_dataset_instance.apply(
    ...
    def __getitem__(self, idx):
        ...
        ret_list = [
            torch.tensor(
                self.df_dataset_instance_standardized.iloc[idx].values,
                dtype=torch.float,
            )
        ]
