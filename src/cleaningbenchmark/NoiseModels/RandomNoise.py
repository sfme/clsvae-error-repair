...
class ImageBlockSaltnPepper(NoiseModel.NoiseModel):
  ...
  def __init__(self,
    ...
    super(ImageBlockSaltnPepper, self).__init__(shape,
                                                probability,
                                                feature_importance,
                                                one_cell_flag)

    # NOTE: once_cell_flag does not apply here. If one wants one block
    #       of pixels then use 'number_blocks' argument above.
    ...
    self.p_pixel = p_pixel # flag
    self.block_size = block_size # always needed

    if p_pixel > 0: # define number of blocks by using p_pixel dirty probability
      self.number_blocks = int(numpy.floor(p_pixel*(self.x_dim*self.y_dim)/float(block_size)))
    else: # or, define number of blocks directly
      self.number_blocks = number_blocks

    self.conv_to_int = conv_to_int

  def _shuffle_indep(self, arr):

    x, y = arr.shape
    rows = numpy.indices((x,y))[0]
    cols = [numpy.random.permutation(y) for _ in range(x)]
  ...
  def corrupt(self, X):
    ...
    # define grid partition (to help space the distribution of block throughout image)
    cell_grid_size = 2*self.block_size # rel. to img
    x_grid_size = int(numpy.floor(float(self.x_dim) / cell_grid_size)) 
    y_grid_size = int(numpy.floor(float(self.y_dim) / cell_grid_size))

    x_grid_idxs = [ff*cell_grid_size for ff in range(x_grid_size)] # rel. to img
    y_grid_idxs = [ff*cell_grid_size for ff in range(y_grid_size)] # rel. to img

    # get random selection of grid cells
    grid_cell_select = numpy.repeat(numpy.arange(x_grid_size*y_grid_size).reshape(1,-1), Ns, axis=0)
    grid_cell_select = self._shuffle_indep(grid_cell_select)[:,:self.number_blocks]

    # get grid cells indexes
    x_grid_selc, y_grid_selc = numpy.unravel_index(grid_cell_select, (x_grid_size, y_grid_size))

    # noise each image with blocks
    for img_idx in range(Ns):

      for x_cell_grid, y_cell_grid in zip(x_grid_selc[img_idx,:], y_grid_selc[img_idx,:]):

        x_pixel_start = x_grid_idxs[x_cell_grid] + numpy.random.randint(self.block_size)
        y_pixel_start = y_grid_idxs[y_cell_grid] + numpy.random.randint(self.block_size)

        img_slicer = numpy.s_[img_idx, x_pixel_start:(x_pixel_start+self.block_size), 
                                       y_pixel_start:(y_pixel_start+self.block_size)]

        rnd_pixel_val = numpy.random.choice([self.min_val,self.max_val], 
                                            p=[self.p_min,1.-self.p_min])

        if self.conv_to_int:
          rnd_pixel_val = rnd_pixel_val.astype(int)

        Y[img_slicer] = numpy.ones((self.block_size,self.block_size))*rnd_pixel_val
