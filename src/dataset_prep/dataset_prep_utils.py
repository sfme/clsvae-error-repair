...
from cleaningbenchmark.NoiseModels.RandomNoise import MixedNoiseModel, ImageSaltnPepper, ImageBlockSaltnPepper, ImageAdditiveGaussianNoise
...
def dirtify_and_save_mixed_simple(df_data, run_stats, path_to_folder):
    ...
    run_stats["num_model"]["int_cast_array"] = \
    run_stats["num_model"]["int_cast_array"].tolist()
...
def dirtify_and_save_image_data(array_data, run_stats, path_to_folder, shape_dim=3,
    ...
    elif run_stats["type"]=="BlockSaltnPepper":

        # block (pixels) Salt and Pepper Noise
        noise_mdl = ImageBlockSaltnPepper(array_data.shape,
                                          probability=run_stats["p_img"],
                                          one_cell_flag=False,
                                          min_val=run_stats["min_val"],
                                          max_val=run_stats["max_val"],
                                          p_min=run_stats['p_min'],
                                          p_pixel=run_stats['p_pixel'],
                                          block_size=run_stats['block_size'],
                                          number_blocks=run_stats['number_blocks'],
                                          conv_to_int=run_stats['conv_to_int'])
