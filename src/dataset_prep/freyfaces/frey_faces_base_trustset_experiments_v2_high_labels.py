...
save_folder = (
    f"../../../data/base_experiments_v2/high_labels/frey_faces/{run_noise_option}/"
)

## noise levels
# noise_level_percentages = [15, 25, 35, 45]
noise_level_percentages = [35]

## trust set sweep values
# per_class_points = np.array([5, 10, 25, 50], dtype=int)

# per_class_points = np.array(120, dtype=int)

fraction_labels_stratisfied = 0.8
...
trust_set_total_points = int(fraction_labels_stratisfied * (n_datapoints * 0.8))
trust_set_percent = trust_set_total_points / n_datapoints * 100


# run / new random seed
# name_run = "run_1"  # NOTE: 5 runs. # 1,2,3,4,5
for cur_run in [0]:  # range(5)
    ...
    for noise_level_cur in noise_level_percentages:
        ...
        save_folder_cur = (
            save_folder
            + f"corrupt_level_{noise_level_cur}_percent/fraction_0.8_labels/"
        )
        ...
        if run_noise_option == "simple_systematic_shapes":
            ...
            run_stats = {
                ...
                "trusted_set": {
                    ...
                    "samples_fixed": None,  # per_class_points.tolist()
                    "frac_trusted": fraction_labels_stratisfied,  # 0.10 ignored if samples fixed.
                    "y_class_on": True,
            ...
            run_stats = {
                ...
                "trusted_set": {
                    ...
                    "mc_mode": "stratisfied_v2",  # "fixed_number" "stratisfied_v2", fixed ?
                    "samples_fixed": None,  # per_class_points.tolist()
                    "frac_trusted": fraction_labels_stratisfied,  # ignored if samples fixed.
                    "y_class_on": True,
                    "y_noise_lists_on": True,
                },
